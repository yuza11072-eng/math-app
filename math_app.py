...
def answer_text(ans):
    if len(ans) == 1:
        ...
        joined = "、".join(str(a) for a in sorted(ans))
        return f"x = {joined}"
...
if st.session_state.page == "quiz":
    ...
    if st.button("答え合わせ"):
        if sorted(user_ans) == sorted(st.session_state.ans):
            st.success("⭕ 正解！")
            st.session_state.score += 1
        else:
            st.error(f"❌ 正解は {answer_text(st.session_state.ans)}")
